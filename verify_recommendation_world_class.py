...
def run_verification():
    ...
    try:
        ...
        # We add 'reset-password' to the title to trigger the missing coverage gap for features with no seeded tests
        pr = PullRequest(
            id=pr_uuid,
            repository_id=repo_id,
            github_pr_id=12345,
            number=42,
            title="Stripe billing integration with authentication, checkout logic and reset-password",
            ...
            github_created_at=datetime.datetime.now(datetime.timezone.utc),
            github_updated_at=datetime.datetime.now(datetime.timezone.utc),
        ...
        fragility = FragilityPattern(
            ...
            normalized_pattern_key="billing/checkout.py",
            title="Frequent failures in checkout billing",
            explanation="The checkout billing exhibits severe historical regressions.",
            ...
            evidence_count=6,
            context={"trigger_file": "billing/checkout.py", "related_tests": ["billing_suite::test_stripe_checkout"]}
        )
        db.add(fragility)
        db.commit()
